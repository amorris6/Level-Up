...
Item::Item(string name, int price, int page,
    ...
    equip_button = new Button(pos_x + kEquipXAdj, pos_y + kHeight,
                              kEquipButtonWidth, kButtonHeight, "EQUIP",
                              button_font, equipItem);
...
Item::Item(string name, int price, int page,
           ofVec2f pos,
           shared_ptr<ofxSmartFont> button_font)
    : Item(name, price, page, pos.x, pos.y,
           button_font) {}

Item::Item(string name, int price, int page,
           float pos_x, float pos_y, function<void()> func_when_equipped,
...
function<void()> Item::getFuncWhenEquipped() {
    return func_when_equipped_[0];
}

function<void()> Item::getFuncWhenUnequipped() { return func_when_unequipped_[0]; }
