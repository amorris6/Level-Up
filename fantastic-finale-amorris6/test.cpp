...
int main() {
    ...
    int init_def = 30;
    int init_health = 40;
    float init_crit_chance = 0.05;
    Player player = Player(0, 0, init_gold, init_exp, 
		                   init_atk, init_def, init_health, init_crit_chance);
    ...
    if (player.getPos().x == player.kMoveSpeed && player.getPos().y == player.kMoveSpeed) {
    ...
    if (player.getRect().getX() == player.getPos().x 
		...
		&& player.getRect().getWidth() == Character::kCharWidth / 2 //so the sprite takes up whole width
